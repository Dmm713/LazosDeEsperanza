...
class OrganizacionesDAO {
    ...
    function insert(Organizacion $organizacion): int|bool{
        if(!$stmt = $this->conn->prepare("INSERT INTO organizaciones (email, password, foto, sid) VALUES (?,?,?,?)")){
        ...
        $stmt->bind_param('sssssssss',$nombre, $descripcion, $sitioWeb, $telefono, $email, $password, $direccion, $foto, $sid);
        if($stmt->execute()){
